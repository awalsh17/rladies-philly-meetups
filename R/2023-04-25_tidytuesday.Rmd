...
lm(wheelchair_men ~ log(precipitation + 0.1), data = prep_df) %>% 
        broom::tidy()
lm(wheelchair_men ~ log(precipitation + 0.1) + year, data = prep_df) %>% 
        broom::tidy()
lm(men ~ precipitation, data = prep_df) %>% 
        broom::tidy()
lm(men ~ precipitation + year, data = prep_df) %>% 
        broom::tidy()
```
